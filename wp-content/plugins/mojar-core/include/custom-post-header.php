...
class MsHeaderPost
{
    ...
    {
        $this->name = __('Header', 'mscore');
        $this->slug = 'ms-header';
        $this->plural_name = __('Header', 'mscore');
    ...
    public function register_custom_post_type()
    {
        ...
        $labels = array(
            ...
            'add_new' => sprintf(__('Add New Template', 'mscore'), $this->name),
            'add_new_item' => sprintf(__('Add New %s', 'mscore'), $this->name),
            'edit_item' => sprintf(__('Edit %s', 'mscore'), $this->name),
            'new_item' => sprintf(__('New %s', 'mscore'), $this->name),
            'all_items' => sprintf(__('All Templates', 'mscore'), $this->plural_name),
            'view_item' => sprintf(__('View %s', 'mscore'), $this->name),
            'search_items' => sprintf(__('Search %s', 'mscore'), $this->name),
            'not_found' => sprintf(__('No %s found', 'mscore'), strtolower($this->name)),
            'not_found_in_trash' => sprintf(__('No %s found in Trash', 'mscore'), strtolower($this->name)),
            'parent_item_colon' => '',
            'menu_name' => $this->name,
        );
        ...
        if (!$cpt_support) {
            $cpt_support = ['page', 'post', 'ms-header', 'ms-footer', 'elementor_disable_color_schemes']; //create array of our default supported post types
