	...
	if (! function_exists('tutor_kses_allowed_html')) {
		function tutor_kses_allowed_html($allowed_tags, $context)
		{
			...
			$atts = array(
				...
				'title',
				'multiple',
				'tutor-hide-course-single-sidebar',
				'href'
			);
...
<!--(.|\s)*?-->/', '', $content);
	...
	if (! function_exists('tutor_closeable_alert_msg')) {
		...
		?>
		<div
			class="tutor-alert tutor-<?php echo esc_attr($alert); ?> tutor-mb-12 tutor-alert tutor-success tutor-mb-12 tutor-d-flex tutor-align-center tutor-justify-between">
			<span>
				<?php echo is_array($allowed_tags) && count($allowed_tags) ? wp_kses($message, $allowed_tags) : esc_html($message); ?>
			</span>
			<span class="tutor-icon-times" area-hidden="true" onclick="this.closest('div').remove()"
			...
			<div class="tutor-snackbar">
				...
				<div>
					<?php foreach ($action_buttons as $attr => $button) : ?>
						<a <?php foreach ($button as $attr => $value) : ?> <?php if (! empty($value)) : ?>
							<?php echo esc_attr($attr) . '="' . esc_attr($value) . '" '; ?> <?php endif; ?>
							<?php endforeach; ?>>
							<?php echo esc_html(isset($button['title']) ? $button['title'] : ''); ?>
						</a>
					<?php endforeach; ?>
					<span class="tutor-icon-times" area-hidden="true"
						onclick="this.closest('#tutor-reuseable-snackbar').remove()" style="cursor: pointer;"></span>
				</div>
			</div>
		</div>
<?php
