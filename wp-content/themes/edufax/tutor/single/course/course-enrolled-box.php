...
<div class="tutor-price-preview-box">
    ...
    <div class="tutor-lead-info-btn-group">
        ...
        if ( $wp_query->query['post_type'] !== 'lesson') {
            ...
            if ( $lesson_url ) { 
                $button_class = 'e-btn e-btn-7 w-100' . ($retake_course ? ' tutor-course-retake-button' : '');
                ?>
                <a href="<?php echo esc_url($lesson_url); ?>" class="<?php echo esc_attr($button_class); ?>" data-course_id="<?php echo get_the_ID(); ?>">
                    <?php
                        if($retake_course) {
                            _e( 'Retake This Course', 'educal' );
                        } else if( $completed_percent <= 0 ){
                            _e( 'Start Course', 'educal' );
                        } else {
                            _e( 'Continue Course', 'educal' );
                        }
                    ?>
                </a>
                <?php 
    ...
    <?php do_action('educal_course_enroll_info_list'); ?>
    ...
    <div class="tutor-single-course-segment  tutor-course-enrolled-wrap">
        <p>
            ...
            <?php
                ...
                echo sprintf(__('You have been enrolled on %s.', 'educal'),  "<span>". date_i18n(get_option('date_format'), strtotime($enrolled->post_date)
                    )."</span>"  );
                ?>
        </p>
