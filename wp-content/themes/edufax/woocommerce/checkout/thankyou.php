...
<div class="tp-order-area woocommerce-order">
		...
		<div class="tp-order-inner">
			...
			<div class="row gx-0 align-items-center">
				<?php if ($order->has_status('failed')) : ?>
					...
					<div class="col-xl-12">
						<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed"><?php esc_html_e('Unfortunately your order cannot be processed as the originating bank/merchant has declined your transaction. Please attempt your purchase again.', 'edufax'); ?></p>

						<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed-actions">
							<a href="<?php echo esc_url($order->get_checkout_payment_url()); ?>" class="button pay"><?php esc_html_e('Pay', 'edufax'); ?></a>
							<?php if (is_user_logged_in()) : ?>
								<a href="<?php echo esc_url(wc_get_page_permalink('myaccount')); ?>" class="button pay"><?php esc_html_e('My account', 'edufax'); ?></a>
							<?php endif; ?>
						</p>
					</div>
						...
						<div class="tp-order-details">
							<div class="tp-order-details-top text-center mb-70">
								...
								<div class="tp-order-details-content">
									<h3 class="tp-order-details-title"><?php echo esc_html__('Your Order Confirmed', 'edufax'); ?></h3>
									<p class="woocommerce-notice woocommerce-notice--success woocommerce-thankyou-order-received"><?php echo apply_filters('woocommerce_thankyou_order_received_text', wp_kses_post('We will send you a shipping confirmation email as soon <br> as your order ships ', 'edufax'), $order); ?></p>
								</div>
							</div>
								...
								<div class="row">
									<div class="col-sm-6">
										<div class=" tp-order-details-item">
											<h4><?php echo esc_html__('Order Date:', 'edufax'); ?></h4>
									...
									<?php if (is_user_logged_in() && $order->get_user_id() === get_current_user_id() && $order->get_billing_email()) : ?>
										<div class="col-sm-6">
											<div class="woocommerce-order-overview__date date tp-order-details-item">
												<h4><?php echo esc_html__('Email:', 'edufax'); ?></h4>
									...
									<?php if (!empty($order->get_order_number())) : ?>
										<div class="col-sm-6">
											<div class="woocommerce-order-overview__order order tp-order-details-item">
												<h4><?php echo esc_html__('Order Number:', 'edufax'); ?></h4>
												<p><?php echo esc_html__('#', 'edufax');
									...
									<?php if (!empty($order->get_payment_method_title())) : ?>
										<div class="col-sm-6">
											<div class="woocommerce-order-overview__payment-method method tp-order-details-item">
												<h4><?php echo esc_html__('Payment Method:', 'edufax'); ?></h4>
	...
	<?php else : ?>
		...
		<p class="woocommerce-notice woocommerce-notice--success woocommerce-thankyou-order-received"><?php echo apply_filters('woocommerce_thankyou_order_received_text', esc_html__('Thank you. Your order has been received.', 'edufax'), null); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
																										?></p>
